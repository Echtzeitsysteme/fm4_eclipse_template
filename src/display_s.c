...
void printPattern_s(const uint16_t backgroundColor,
		...
		for (j = 0; j < 8; j++, line >>= 1) { // Draw in y-direction
			if (line & 0x1) {
				...
				if (size == 1)
					...
					cppp_fillRect(x + (i * size), y - ((j + 1) * size) + 1,
							size, size, color); //keep in mind cppp_fillRect wants the lower left corner
			} else if (bg != color) {
				if (size == 1)
