...
en_result_t Adc_SwTriggerScan( volatile stc_adcn_t* pstcAdc )
{
	...
	//trace_printf("1119 Adc_SwTriggerScan \n");
...
uint32_t Adc_ReadScanFifo( volatile stc_adcn_t* pstcAdc )
{
   ...
   // trace_printf("test pstcAdc->SCFD = %d \n", pstcAdc->SCFD);
    return pstcAdc->SCFD;
} // Adc_ReadScanFifo
 ...
 ******************************************************************************/
uint32_t  Adc_GetScanData(volatile stc_adcn_t* pstcAdc,
                          uint32_t u32FifoData )
{
  if(0u == pstcAdc->ADSR_f.FDAS) //  ADSR_f.FDAS
    {
    	//trace_printf("Adc_GetScanData u32FifoData x =  %d \n",u32FifoData);
    	// trace_printf("Adc_GetScanData u32FifoData >> 20u  %d \n",(u32FifoData >> 20u) & 0xFFFu );
        return ((u32FifoData >> 20u) & 0xFFFu);
    }
    else
    {
    	//trace_printf("Adc_GetScanData u32FifoData y =  %d \n",u32FifoData);
    	// trace_printf("Adc_GetScanData u32FifoData >> 16u  %d \n",(u32FifoData >> 16u) & 0xFFFu);
        return ((u32FifoData >> 20u) & 0xFFFu);
    }
