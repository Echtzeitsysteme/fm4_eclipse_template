...
void cppp_getAnalogValues(uint16_t *analog11, uint16_t *analog12,
		uint16_t *analog13, uint16_t *analog16, uint16_t *analog17,
		uint16_t *analog19, uint16_t *analog23) {
	uint32_t u32Data = 0u;     // 32 Bit Data buffer for measurements

	// Start single ADC Scan
	Adc_SwTriggerScan(&ADC0);

	// Due to not simulating a fifo buffer register, data for joystick is transmitted by normal registers
	// 19th and 18th bit of u32Data used to decide whether right or left joystick or other getAnalogValues call
	// (normally these bits are disabled -> see json and hardware description of s6e2cc)

	cppp_adcPollData(&u32Data);
	if (((u32Data & (1 << 18)) >> 18) == 1
			&& ((u32Data & (1 << 19)) >> 19) == 0) {

		*analog23 = 190;

		*analog19 = (u32Data) & 0xFFu;
		*analog16 = (u32Data >> 20) & 0xFFFu;

		*analog13 = 190;

	} else if (((u32Data & (1 << 18)) >> 18) == 0
			&& ((u32Data & (1 << 19)) >> 19) == 0) {

		*analog23 = (u32Data) & 0xFFu;
		*analog19 = 190;

		*analog16 = 190;
		*analog13 = (u32Data >> 20) & 0xFFFu;

	} else {

		// Default assign of analog values
		// Assignment order is important! (11->12->23...)
		...
		for (i = 0; i < 7; i++) {
			...
			switch (i) {
			case 0:
				*analog11 = (uint32_t) (Adc_GetScanData(&ADC0, u32Data));
				break;
			case 1:
				*analog12 = (uint32_t) (Adc_GetScanData(&ADC0, u32Data));
				break;
			case 2:
				*analog23 = (uint8_t) (Adc_GetScanData(&ADC0, u32Data));// >> 4);
				break;
			case 3:
				*analog19 = (uint8_t) (Adc_GetScanData(&ADC0, u32Data));// >> 4);
				break;
			case 4:
				*analog17 = (uint8_t) (Adc_GetScanData(&ADC0, u32Data) >> 4);
				break;
			case 5:
				*analog16 = (uint32_t) (Adc_GetScanData(&ADC0, u32Data));
				break;
			case 6:
				*analog13 = (uint8_t) (Adc_GetScanData(&ADC0, u32Data));// >> 4);
				break;
			}
		}
	}
...
uint16_t cppp_mapValueRange(uint16_t x, uint16_t in_min, uint16_t in_max,
...
uint16_t cppp_readTouchX(void) {
	...
	cppp_getAnalogValues(&analog11, &analog12, &analog13, &analog16, &analog17,
			&analog19, &analog23);
	sample = analog11;
