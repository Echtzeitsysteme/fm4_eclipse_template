...
void bin(unsigned n) {
	...
	for (i = 1 << 31; i > 0; i = i / 2)
		(n & i) ? trace_printf("1") : trace_printf("0");
	trace_printf(" \n");
}
...
void cppp_write32(char r, long d) {
	...
	//  LCD_CD = 0u;
//	trace_printf("28 cppp_write32  \n");
	cppp_write8(r);
	//   LCD_CD = 1u;
	// trace_printf("43 cppp_write32 d >> 24 = %x  \n", d >> 24);
	cppp_write8(d >> 24);
	//   trace_printf("45 cppp_write32 d >> 16 = %x  \n", d >> 16);
	cppp_write8(d >> 16);
	//    trace_printf("45 cppp_write32 d >> 8  = %x  \n", d >> 8);
	cppp_write8(d >> 8);
	//  trace_printf("47 cppp_write32 d = %x  \n", d );
	cppp_write8(d);
...
void cppp_setLR(void) {
	LCD_CS = 0u;
	cppp_writeRegisterPair(HX8347G_COLADDREND_HI, HX8347G_COLADDREND_LO,
			480 - 1);
	cppp_writeRegisterPair(HX8347G_ROWADDREND_HI, HX8347G_ROWADDREND_LO,
			320 - 1);
	LCD_CS = 1u;
}
...
void cppp_floodLCD(int color, long len) {
	...
	volatile unsigned char i, hi = color >> 8, lo = color;
